...
$("#loan-options .option").each(function(){
    ...
    if( $(window).width()> 1000){
      $(this).children('div:eq(1)').attr('style', 'height: calc(3.5rem*3); overflow: hidden;')
    }
    if( $(window).width()<= 1000){
      $(this).children('div:eq(1)').attr('style', 'height: calc(3.5rem*6); overflow: hidden;')
    }
    if( $(window).width()<= 500){
      $(this).children('div:eq(1)').attr('style', 'height: calc(3.5rem*7); overflow: hidden; ')
    }
  ...
  $("#loan-options .expand").on('click', (function(){
    ...
    if($(this).hasClass('0')){
      ...
      if( $(window).width()>1000 && $(this).hasClass('1')){
        $(this).siblings('div:eq(1)').attr('style', 'height: calc(3.5rem*3); overflow: hidden;')
        $(this).removeClass('1').addClass('0')
    }if( $(window).width()<= 1000 && $(this).hasClass('1')){
      $(this).siblings('div:eq(1)').attr('style', 'height: calc(3.5rem*6); overflow: hidden;')
      $(this).removeClass('1').addClass('0')
    }if( $(window).width()<= 500 && $(this).hasClass('1')){
      $(this).siblings('div:eq(1)').attr('style', 'height: calc(3.5rem*9); overflow: hidden;')
      $(this).removeClass('1').addClass('0')
    } 
  }
  //   $(this).siblings("div").slice(2).slideToggle()
  ...
  //   $(this).removeClass('1').addClass('0')
  // }
  // console.log($(this).children('img').attr())
